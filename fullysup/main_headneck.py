...
from src.models.lightunetr_large import LightUNETRLarge
        ...
        metric.update(
            {
                ...
                f"Train/head {metric_name}": float(batch_acc[0]),
                f"Train/neck {metric_name}": float(batch_acc[1]),
            }
        )
    accelerator.print(
        ...
        metric.update(
            {
                ...
                f"Val/head {metric_name}": float(batch_acc[0]),
                f"Val/neck {metric_name}": float(batch_acc[1]),
            }
        )
    accelerator.print(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train LightUNETR for HeadNeck")
    parser.add_argument(
        "--save_dir",
        type=str,
        default="/data_hdd/xyliu/lightunetr_experiments",
    ...
    parser.add_argument(
        ...
        type=str,
        default="LightUNETR",
        help="Model to use for training, default is LightUNETR",
    )
    parser.add_argument(
        "--resume",
        action="store_true",
    ...
    # load yml
    config = EasyDict(
        yaml.load(open("config_headneck_lightunetr.yml", "r", encoding="utf-8"), Loader=yaml.FullLoader)
    )
    if config.is_headneck == True:
        config = config.headneck
        data_flag = "headneck"
    else:
        raise ValueError("Please set the correct config flag for the dataset you are using.")
    config.trainer.batch_size = 1
    config.trainer.lr = 0.0002
    same_seeds(50)
    
    logging_dir = os.path.join(arg.save_dir, f"taskheadneck_{arg.model}/logs/{config.finetune.checkpoint}/" + str(datetime.now()))
    ...
    if arg.model == "LightUNETRLarge":
        model = LightUNETRLarge(1, 2)
    elif arg.model == "LightUNETR":
        model = LightUNETR(1, 2)
    else:
        raise ValueError(f"Model {arg.model} not recognized. Please choose from LightUNETRLarge, LightUNETR")
    ...
    # resume training
    if arg.resume:
        accelerator.print(f"Resume training from checkpoint: {config.finetune.checkpoint}")
    ...
    for epoch in range(starting_epoch, config.trainer.num_epochs):
        ...
        if mean_acc > best_acc:
            accelerator.save_state(
                output_dir=f"{arg.save_dir}/taskheadneck_{arg.model}/model_store/{config.finetune.checkpoint}/best"
            )
            best_acc = mean_acc
        ...
        if epoch % 200 == 0:
            accelerator.save_state(
                output_dir=f"{arg.save_dir}/taskheadneck_{arg.model}/model_store/{config.finetune.checkpoint}/epoch_{epoch}"
            )

    # save final model
    accelerator.save_state(
        output_dir=f"{arg.save_dir}/taskheadneck_{arg.model}/model_store/{config.finetune.checkpoint}/final"
    )
