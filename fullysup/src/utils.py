...
def resume_train_state(
    ...
    try:
        ...
        def load_model_safetensor(ckpt_path, model, accelerator):
            if ckpt_path is not None:
                accelerator.print(f"Load model from {ckpt_path}")
                ckpt = load_file(ckpt_path, device='cpu')
                # add module. prepend to keys if necessary
                if not any(key.startswith("module.") for key in ckpt.keys()):
                    ckpt = {f"module.{k}": v for k, v in ckpt.items()}
                model.load_state_dict(ckpt, strict=True)
            return model
        # model = load_pretrain_model(dirs[-1] + "/pytorch_model.bin", model, accelerator)
        model = load_model_safetensor(dirs[-1] + "/model.safetensors", model, accelerator)
