...
if __name__ == "__main__":
    ...
    # load yml
    config = EasyDict(
        yaml.load(open("config_abdomen_lightunetr_large.yml", "r", encoding="utf-8"), Loader=yaml.FullLoader)
    )
    ...
    logging_dir = os.path.join(save_dir, "taskabdomen_lightunetr_large", "logs", config.finetune.checkpoint, str(datetime.now().strftime('%Y%m%d_%H%M%S')))
    ...
    for epoch in range(starting_epoch, config.trainer.num_epochs):
        ...
        if mean_acc > best_acc:
            accelerator.save_state(
                output_dir=f"{save_dir}/taskabdomen_lightunetr_large/model_store/{config.finetune.checkpoint}/best"
            )
            best_acc = mean_acc
        ...
        if epoch % 50 == 0:
            accelerator.save_state(
                output_dir=f"{save_dir}/taskabdomen_lightunetr_large/model_store/{config.finetune.checkpoint}/epoch{epoch+1}"
            )

    # save final model
    accelerator.save_state(
        output_dir=f"{save_dir}/taskabdomen_lightunetr_large/model_store/{config.finetune.checkpoint}/final"
    )
