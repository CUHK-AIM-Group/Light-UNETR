...
def var_all_case_LA(model, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, val_list='test'):
   
    with open('/data/xyliu/UA-MT/data/{}.list'.format(val_list), 'r') as f:
        image_list = f.readlines()
    image_list = ["/data/xyliu/UA-MT/data/2018LA_Seg_Training Set/" + item.replace('\n', '') + "/mri_norm2.h5" for item in image_list]
...
def var_all_case_Pancreas(model, num_classes, patch_size=(96, 96, 96), stride_xy=16, stride_z=4, val_list='test'):
   
    with open('/data/xyliu/codes/{}.list'.format(val_list), 'r') as f:
        image_list = f.readlines()
    image_list = ["/data/xyliu/codes/data/" + item.replace('\n', '') + ".h5" for item in image_list]
    loader = tqdm(image_list)
    total_dice = 0.0
    for image_path in loader:
        h5f = h5py.File(image_path, 'r')
...
def var_all_case_Brats(model, num_classes, patch_size=(96, 96, 96), stride_xy=64, stride_z=64, val_list='val'):
   
    with open('/data/xyliu/brats2019/val.list', 'r') as f:
        image_list = f.readlines()
    image_list = ["/data/xyliu/brats2019/data/" + item.replace('\n', '') + ".h5" for item in image_list]
    loader = tqdm(image_list)
    print("total {} cases for val".format(len(image_list)))
    total_dice = 0.0
    for image_path in loader:
        h5f = h5py.File(image_path, 'r')
        image = h5f['image'][:]
...
def test_all_case(model, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, save_result=True, test_save_path=None, preproc_fn=None, metric_detail=0, nms=0):
    ...
    if test_save_path is not None:
        metrics_text = open(test_save_path + 'metrics.txt', 'w')
    ...
    for image_path in loader:
        ...
        if metric_detail:
            print('%s,\t%.5f, %.5f, %.5f, %.5f' % (image_path, single_metric[0], single_metric[1], single_metric[2], single_metric[3]))
        if test_save_path is not None:
            metrics_text.write('%s,\t%.5f, %.5f, %.5f, %.5f\n' % (image_path, single_metric[0], single_metric[1], single_metric[2], single_metric[3]))
    ...
    print("Results saved to: ", test_save_path)
